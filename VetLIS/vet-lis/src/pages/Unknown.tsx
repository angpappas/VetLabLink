...
import { Button, Input, InputRef, Result, Space, Table, TablePaginationConfig } from "antd";
import { ColumnType, ColumnsType, FilterConfirmProps, FilterValue, SorterResult, TableCurrentDataSource } from "antd/es/table/interface";
...
type DataIndex = keyof UnknownDataType;
...
const Unknown: React.FC = (props) => {
    ...
    const [messages, setMessages] = useState<DataPage<UnknownDataType> | null>(null);
    ...
    const getColumnSearchProps = (dataIndex: DataIndex): ColumnType<UnknownDataType> => ({
    ...
    const columns: ColumnsType<UnknownDataType> = [
        {
            title: "Test time",
            dataIndex: "testTime",
            sorter: {
        ...
        },
        {
            ...
            dataIndex: "birthDate",
            sorter: {
                compare: (a: UnknownDataType, b: UnknownDataType) => {
    ...
    const fetchData = async () => {
        try {
            ...
            const tmpMessages: DataPage<UnknownDataType> = {
                currentPage: data.currentPage,
                pageSize: data.pageSize,
                totalRecords: data.totalRecords,
                data: data.data.map((x) => readUnknown(x, context?.testParameterSettings!))
            };
    ...
    useEffect(() => {
        context?.setCurrentPage("unknown");
    ...
    const handleTableChange = (
        ...
        sorter: SorterResult<UnknownDataType> | SorterResult<UnknownDataType>[],
        extra: TableCurrentDataSource<UnknownDataType>
    ) => {
        setTableParams({
            pagination,
            ...filters,
    ...
    return (
        <div>
            <h2 className="titles">Unknown</h2>
