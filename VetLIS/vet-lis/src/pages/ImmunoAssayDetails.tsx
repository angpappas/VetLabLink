...
const ImmunoAssayDetails: React.FC = (props) => {
    const { messageId } = useParams();
    const location = useLocation();
    const [message, setMessage] = useState<ImmunoAssayDataType | null>(null);
                        ...
                        <View style={styles.container}>
                            ...
                            <View style={styles.details}>
                                <View style={styles.tableRow}>
                                    ...
                                    <TextPdf style={{ fontWeight: "bold" }}>&nbsp;{message?.patientName?.toLocaleUpperCase()}</TextPdf>
                                </View>
                                <View style={styles.tableRow}>
                                        ...
                                        <View style={styles.dataTable}>
                                            ...
                                            >
                                                <View style={styles.tableCol1}>
                                                    <TextPdf style={[styles.tableCell, { maxLines: 1 }]}>Item</TextPdf>
                                                </View>
                                                <View style={styles.tableCol2}>
                                                    <TextPdf style={styles.tableCell}>Sub-Item</TextPdf>
                                                </View>
                                            ...
                                            {message?.measurements.map((x) => (
                                                <View style={styles.tableRow}>
                                                    <View style={styles.tableCol1}>
                                                        <TextPdf style={[styles.tableCell, { backgroundColor: "#E4E4E4" }]}>{x.item}</TextPdf>
                                                    </View>
                                                    <View style={styles.tableCol2}>
                                                        <TextPdf style={styles.tableCell}>{x.subItem}</TextPdf>
                                                    </View>
                                                    <View style={styles.tableCol3}>
    ...
    const fetchData = async (id: number | string) => {
        ...
        setMessage(readImmunoassay(data.message, context?.testParameterSettings!));
    ...
    useEffect(() => {
        if (messageId) {
            fetchData(messageId);
            context?.setCurrentPage("immunoassay");
        }
        ...
        <>
            <div>
                ...
                >
                    ...
                    <Descriptions.Item span={2} label={<strong>Sample type</strong>}>
                        {message.sampleType}
                    </Descriptions.Item>
                    {message.measurements.map((x) => (
                        <Descriptions.Item key={x.label} label={<strong>{x.subItem}</strong>}>
                            <EditableDescription
                                label={x.label}
                                value={x.value}
                                updateCallback={updateOverrides}
                ...
                >
                    <Link
                        to={!hasPrevious() ? "" : `/immunoassay/${hasPrevious()}`}
                    ...
                    <Link
                        to={!hasNext() ? "" : `/immunoassay/${hasNext()}`}
                        state={location.state}
...
export default ImmunoAssayDetails;
